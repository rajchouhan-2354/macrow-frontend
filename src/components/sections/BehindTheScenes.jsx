  ...
  return (
    <section className="py-24 bg-macrow-white text-macrow-textDark relative border-t border-b border-macrow-borderLight overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* --- FILM STRIP HORIZONTAL REEL CONTAINER (Secondary 30% Black Container) --- */}
        <div className="bg-macrow-black text-macrow-white rounded-2xl md:rounded-3xl border-2 border-macrow-borderDark shadow-2xl p-6 md:p-8 relative overflow-hidden mb-12">
          ...
          <div className="w-full flex items-center justify-between opacity-30 pb-4 border-b border-macrow-borderDark/60 mb-6 overflow-hidden">
            {Array.from({ length: 24 }).map((_, i) => (
              <div key={i} className="w-3 h-3 rounded-full bg-macrow-white border border-macrow-darker" />
            ))}
          </div>
        ...
        <div className="flex justify-center mb-20">
          <div className="inline-flex items-center gap-6 px-6 py-3 rounded-full bg-macrow-lightSurface border border-macrow-borderLight shadow-card-light text-xs font-mono">
            <span className="flex items-center gap-2 text-macrow-black font-semibold">
              <span className="w-2 h-2 rounded-full bg-macrow-red" />
              24 FPS
            </span>
            <span className="text-macrow-borderLight">|</span>
            <span className="flex items-center gap-2 text-macrow-black font-semibold">
              <span className="w-2 h-2 rounded-full bg-macrow-red" />
              5-7 Days Sprint
            </span>
        ...
        {/* --- STORYBOARD DEVELOPMENT PROCESS MATRIX CANVAS (Secondary 30% Black Container) --- */}
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-6">
            <span className="text-xs font-mono text-macrow-textDarkMuted uppercase tracking-widest font-semibold">
              A glimpse into our storyboard development process
            </span>
          </div>

          <div className="bg-macrow-black text-macrow-white rounded-3xl border-2 border-macrow-borderDark p-6 md:p-8 shadow-2xl overflow-hidden relative">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {storyboardNodes.map((cat) => (
                <div key={cat.category} className="space-y-4">
                  <div className="bg-macrow-surface px-3 py-1.5 rounded border border-macrow-borderDark text-xs font-mono font-bold text-macrow-red text-center">
                    {cat.category}
                  </div>
                  ...
                  <div className="space-y-3">
                    {cat.shots.map((shot) => (
                      <div
                        key={shot.title}
                        className="group relative aspect-[16/10] rounded-lg overflow-hidden bg-macrow-surface border border-macrow-borderDark hover:border-macrow-red transition-all duration-300 shadow"
                      >
                        <img
                          src={shot.img}
                          alt={shot.title}
                        ...
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-macrow-black via-transparent to-transparent opacity-80" />
                        <span className="absolute bottom-1.5 left-2 text-[10px] font-mono text-macrow-white font-semibold">
                          {shot.title}
                        </span>
                      </div>
                    ))}
          ...
          <p className="mt-4 text-center text-xs font-serif italic text-macrow-textDarkMuted">
